...
int keycodeFromBindName(char* bindName) {
    for (long unsigned int i=0; i<(sizeof(namedKeys)/sizeof(struct namedKeyCode)); i++) {
        if (strncmp(namedKeys[i].keyName, bindName, MAX_BIND_NAME_LEN) == 0) {
...
char* keyNameFromKeyCode(int keyCode) {
    for (long unsigned int i=0; i<(sizeof(namedKeys)/sizeof(struct namedKeyCode)); i++) {
        if (namedKeys[i].keyCode == keyCode) {
            return namedKeys[i].keyName;
        }
    }
    return namedKeys[0].keyName; // UNKNOWN key
}
